...
btnAdd.addEventListener("click", async () => {
  ...
  try {
    ...
    let contactToAdd = {
      ...
      birthdate: new Date(inputBirthdate.value).toLocaleDateString("fr-BE", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      }),
...
const isInList = () => {
  return new Promise((resolve, reject) => {
    ...
    let contactSearched = contacts.find(
      (contact) =>
        contact.lastname === inputLastname.value &&
        contact.firstname === inputFirstname.value
    );
...
const displayContacts = () => {
  if (contacts.length == 0) {
    showContacts.innerHTML =
      '<tr><td colspan="4">Vous n\'avez pas encore de contacts</td></tr>';
  } else {
    showContacts.innerHTML = "";
