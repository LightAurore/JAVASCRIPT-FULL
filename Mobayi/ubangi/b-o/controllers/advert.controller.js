...
const advertController = {
    ...
    addCommentToadvert_POST: async (req, res) => {
        ...
        let data;
        try {
            data = await advertCommentValidator.validate(req.body);
        }
        catch (error) {
            console.log(error);
            ...
            res.redirect(`/advert/detail/${req.body.slug}`);
            return;
        }

        // Add comment into DB
        const commentAdded = await advertService.addComment(data.slug, data.comment, userId);
