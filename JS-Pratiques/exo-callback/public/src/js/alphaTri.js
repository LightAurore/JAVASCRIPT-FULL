...
function TriParLettre(strA, strB) {
  ...
  let normA = strA.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  let normB = strB.normalize("NFD").replace(/[\u0300-\u036f]/g, "");

  let iter = strA.length > strB.length ? strB.length : strA.length;

  let i = 0;
  while (i < iter) {
    let sA = normA.charCodeAt(i);
    let sB = normB.charCodeAt(i);
