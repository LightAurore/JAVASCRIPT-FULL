...
btnFiltrerNom.addEventListener("click", function () {
  ...
  const element_filtre_selection = liste_eleves_ordre_entrer.filter(function (
    ...
    console.log(parseInt(elements_filtres.value));
    return name.length <= parseInt(elements_filtres.value);
  });
  for (let i = 0; i < element_filtre_selection.length; i++) {
    let user = afficherUsers(element_filtre_selection[i]);
  ...
  if (liste_eleves_ordre_entrer.length > 0) {
    ...
    nombre_filtres.innerText = `${liste_eleves_ordre_entrer.length} stagiaire${
      liste_eleves_ordre_entrer.length > 1 ? "s" : ""
    } trouvé${liste_eleves_ordre_entrer.length > 1 ? "s" : ""}.`;
  }
});
