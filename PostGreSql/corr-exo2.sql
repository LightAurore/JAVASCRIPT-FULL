...
FROM
	student
WHERE
	section_id NOT IN (1010, 1020, 1110);
...
-- étudiants qui ont un nom de famille pour lequel la troisième lettre est un « n » et 
-- qui ont obtenu un résultat annuel supérieur à 10
...
-- résultats annuels décroissants de tous les étudiants qui ont obtenu un résultat annuel 
...
-- Exercice 2.2.9 – Ecrire une requête pour présenter le nom, l’id de section et le résultat
-- annuel classé par ordre croissant sur la section de tous les étudiants appartenant 
-- aux sections 1010 et 1020 ayant un résultat annuel qui n’est pas compris entre 12 et 18
...
FROM
	student
WHERE
	section_id IN (1010, 1020)
	AND year_result NOT BETWEEN 12 AND 18
ORDER BY
	section_id ASC;

-- Exercice 2.2.10 – Ecrire une requête pour présenter le nom, l’id de section et le 
-- résultat annuel sur 100 (nommer la colonne « Résultat sur 100 ») classé par ordre 
-- décroissant du résultat de tous les étudiants appartenant aux sections commençant 
-- par 13 et ayant un résultat annuel sur 100 inférieur ou égal à 60
...
FROM
	student
WHERE
	section_id::VARCHAR LIKE '13%'
	-- CAST(section_id AS VARCHAR) LIKE '13%'
	AND (year_result * 5) <= 60
ORDER BY
	"Résultat sur 100" DESC;
